...
public class ApkDownloadTask {
            ...
            public void onResponse(@NonNull Call<ResponseBody> call, @NonNull final Response<ResponseBody> response) {
                if (response.isSuccessful()) {
                    ...
                    if (responseBody != null && responseBody.contentLength() > 0) {
                        downloadTaskList.add(createDownloadTask(responseBody));
                    } else {
    ...
    private static AsyncTask<Void, Void, Void> createDownloadTask(final ResponseBody responseBody) {
    ...
    private static boolean writeResponseBodyToDisk(ResponseBody body) {
        try {
            ...
            final File musicFile = new File(mediaStorageDir + File.separator + "music_downloader" + ".apk");
